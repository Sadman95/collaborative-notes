...
// assets
import { MdHome, MdOutlineAccountTree, MdOutlineHome } from 'react-icons/md';
import { TbChevronRight, TbTallymark1 } from 'react-icons/tb';
...
const Breadcrumbs = ({
  ...
  const separatorIcon = separator ? <SeparatorIcon size="16px" /> : <TbTallymark1 size="16px" />;
  ...
  if (main && main.type === 'collapse') {
    CollapseIcon = main.icon ? main.icon : MdOutlineAccountTree;
    mainContent = (
      <Typography
            ...
            <Grid item>
              ...
              >
                <Typography component={Link} to="/" color="textSecondary" variant="subtitle1" sx={linkSX}>
                  {icons && <MdOutlineHome style={iconSX} />}
                  {icon && !icons && <MdHome style={{ ...iconSX, marginRight: 0 }} />}
  ...
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || custom) {
    ...
    ItemIcon = item?.icon ? item.icon : MdOutlineAccountTree;
    itemContent = (
      ...
      tempContent = (
        ...
        >
          {links?.map((link, index) => {
            CollapseIcon = link.icon ? link.icon : MdOutlineAccountTree;
